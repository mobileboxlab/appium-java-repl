...
public class AndroidCommands extends AppiumCommands<AndroidDriver, AndroidElement> {
	...
	@Override
	@CommandRef(
	desc="Start a new Appium session for Android.",
	params={"deviceName -  The kind of mobile device or emulator to use",
	...
	@CommandRef(
	desc = "Find elements by text.", 
	params = { "text -  The visible text displayed in a widget (for example, the text label to launch an app)." }, 
	...
	@CommandRef(
	desc = "Find elements by text that contains a given text.", 
	params = { "text -  The visible text displayed in a widget (for example, the text label to launch an app). "
	...
	params = {"selector - The UISelector expression. E.g: className('android.widget.RelativeLayout').enabled(true).instance(0);"},
	ret = "A list of AndroidElement. This list is empty when no elements are found.")
