...
/**
 * Commands for control an Android device or emulator from outside of Appium
 * code. With these commands you can takes screenshots, set brightness,
 * retrieves devices properties, etc. For provide these functions we uses an ADB
 * client implemented in pure Java: JADB
 *
...
public class AndroidDeviceCommands {
	...
	public AndroidDeviceCommands(final String serial) throws CommandsException {
		...
		try {
			...
			List<JadbDevice> devices = jadb.getDevices().stream()
					.filter(d -> d.getSerial().contains(serial))
	...
	@CommandRef(
	desc = "Set brightness.", 
	params = {"brightness - Value into the range [0,255]."})
	public void setBrightness(final int brightness) throws IOException,
			JadbException {
		device.executeShell("settings", "put", "system",
				"screen_brightness", Integer.toString(brightness));
	}
	...
	@CommandRef(
	desc = " Remove a file.",
	params = {"filePath - The file path to remove."})
	...
	@CommandRef(
	desc = "Take and save a device screenshot to file.",
	params = {"filePath - Full path to file"})
	public void screenshot(final String filePath) throws IOException,
			JadbException {
	...
	public void executeShell(final String command, final String... args) throws IOException, JadbException {
	...
	params = {"propertye -   The property name"},
	ret = "The property value")
	...
	private void keyEvent(final int event) throws IOException, JadbException {
		...
		device.executeShell("input", "keyevent",
