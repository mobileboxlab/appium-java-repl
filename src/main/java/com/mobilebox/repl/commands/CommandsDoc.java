...
public abstract class CommandsDoc {
	...
	/**
	 * Prints all commands in a given class. In order to define command in Java
	 * code, you need to annotate the respective methods with
	 * <code>@CommandRef</code> annotation.
	 * 
	...
	public static void printCommands(Class<?> clazz) {
		...
		ref.append("---> Command: %s").append(SEPARATOR)
				.append("---> Description: %s").append(SEPARATOR)
				.append("---> Parameters: %s").append(SEPARATOR)
				.append("---> Return: %s").append(SEPARATOR);
		...
		for (Method method : clazz.getMethods()) {
			...
			if (Modifier.isPublic(method.getModifiers())) {
				...
				if (def != null) {
					...
					returns = def.ret();
					console(String.format(ref.toString(), method.getName(),
							descOfMethod, Arrays.toString(params), returns));
				}
